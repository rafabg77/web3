const DIAS_NOVEDAD = 5; // cuántos días se muestra el sello "Novedad" desde la fecha del material

function esNovedad(fechaStr) {
  if (!fechaStr) return false;
  const fecha = new Date(fechaStr + 'T00:00:00');
  if (Number.isNaN(fecha.getTime())) return false;
  const diffDias = (Date.now() - fecha.getTime()) / (1000 * 60 * 60 * 24);
  return diffDias >= 0 && diffDias <= DIAS_NOVEDAD;
}
...
async function iniciar() {
  ...
  function construirTarjeta(m) {
    ...
    a.innerHTML = `
      ${esNovedad(m.fecha) ? '<span class="sello-novedad">Novedad</span>' : ''}
